...
import type { InitialDockLightEnvironment, CreateDockLightEnvironmentCommand } from "../(models)/initialdocklightenvironment.ts";
import type { ApiObject } from "../../../models/api-object.ts";
import Box from "@mui/material/Box";
import { useNavigate } from 'react-router';

const AddInitialDockLightStep = () => {
  ...
  const getPossibleDockerProtocols = async () => {
    ...
    if (!result.isSuccess) {
      throw Error(result.errors.toString())
    }
    if (result.result) {
    ...
    <>
      <Stack spacing={1}>
        <Typography variant='body1'>
          To setup Viix's Docker Manager, we need to create an environment. <br />
          This environment will communicate with your local docker engine. <br />

          Based upon your OS, the protocol is predefined.<br />

          You are running <Box component="span" sx={{ fontWeight: 'bold' }}>{initialEnvironment?.environment}</Box>{initialEnvironment?.isRunningInDocker ? ' in docker.' : '.'}
        </Typography>
        ...
        <Stack spacing={1} direction='column' alignItems='start'>
          ...
          <TextField id='connection' fullWidth
        ...
        <Button variant='contained' onClick={saveNewEnvironment}>Temporary save button</Button>
      </Stack>
